...
static bool elf_is_executable(struct elf_header* header){
	return (header->e_type == ET_EXEC) && (header->e_entry >= VARGOOS_PROGRAM_VIRTUAL_ADDRESS);
}
...
//getter for the elf file location in the memory
void* elf_memory(struct elf_file* file){
	return file->elf_memory;
}

//returns elf memory as the elf header
...
/*
	This function returns a particular program header entry.
*/
...
int elf_validate_loaded(struct elf_header* header){
	...
	&& elf_has_program_header(header)) ? STATUS_OK : -EINVFORMAT;
}
